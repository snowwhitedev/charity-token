...
// import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v3.3/contracts/GSN/Context.sol";
...
contract CharityToken is Context, IERC20, Ownable {
    ...
    uint256 private initialSupply = 100000000 * 10**18;
    ...
    constructor(
        ...
        _PROMO_FEE = _promoFee;
        _TAX_FEE = _txFee * 100;
        _BURN_FEE = _burnFee * 100;
        _CHARITY_FEE = _charityFee * 100;
        ORIG_PROMO_FEE = _PROMO_FEE;
    ...
    function increaseAllowance(address spender, uint256 addedValue) public virtual returns (bool) {
        _approve(msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }
    ...
    {
        (uint256 tPromo, uint256 tFee, uint256 tBurn, uint256 tCharity) = _getTBasics(tAmount, _PROMO_FEE, _TAX_FEE, _BURN_FEE, _CHARITY_FEE);
    ...
    {
        uint256 tPromo = ((tAmount.mul(promoFee)).div(_GRANULARITY)).div(100);
        uint256 tFee = ((tAmount.mul(taxFee)).div(_GRANULARITY)).div(100);
        uint256 tBurn = ((tAmount.mul(burnFee)).div(_GRANULARITY)).div(100);
        uint256 tCharity = ((tAmount.mul(charityFee)).div(_GRANULARITY)).div(100);
        return (tPromo, tFee, tBurn, tCharity);
    }
    ...
    function removeAllFee() private {
        if (_PROMO_FEE == 0 && _TAX_FEE == 0 && _BURN_FEE == 0 && _CHARITY_FEE == 0) return;
